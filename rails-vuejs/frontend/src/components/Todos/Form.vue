...
<form @submit.prevent="$emit('input', {...todo})">
  <div class="mt-6 grid grid-cols-1 row-gap-6 col-gap-4 sm:grid-cols-6">
    <div class="sm:col-span-3">
      <label class="block text-sm font-medium leading-5 text-gray-700" for="description">
        Description
      </label>
  ...
  <div class="relative flex items-start mt-6">
    <div class="flex items-center h-5">
      <input v-model="todo.completed" type="checkbox" id="completed" class="form-checkbox h-4 w-4 text-green-600 transition duration-150 ease-in-out" />
    </div>

    <div class="ml-3 text-sm leading-5">
      <label class="font-medium text-gray-700" for="completed">
        Completed
      </label>
    </div>
  </div>
  ...
  <div class="mt-8 border-t border-gray-200 pt-5">
    <div class="flex justify-end">
      <span class="inline-flex rounded-md shadow-sm">
        <RouterLink :to="{ name: 'index-todo' }" class="py-2 px-4 border border-gray-300 rounded-md text-sm leading-5 font-medium text-gray-700 hover:text-gray-500 focus:outline-none focus:border-green-300 focus:shadow-outline-green active:bg-gray-50 active:text-gray-800 transition duration-150 ease-in-out">
          Back
        </RouterLink>
      </span>
